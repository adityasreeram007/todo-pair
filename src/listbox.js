...
componentDidMount(){
    axios.post("https://todo-pair-one.herokuapp.com/getdata",{"teamname":teamname})
    .then((response) => {
...
addList(){
    ...
    if (title==='' || desc===''){
        ...
        axios.post("https://todo-pair-one.herokuapp.com/putdata",{"title":title,"desc":desc,'team':teamname,'user':user})
        .then((response) => {
            if (response.status){
